...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                entropy = -torch.sum(probs * log_probs, dim=-1)  # (num_tokens,)
                masked_entropy  = entropy * loss_mask.view(-1).to(entropy.device)
                mean_entropy = masked_entropy.sum() / (loss_mask.sum() + 1e-8)
            ...
            if do_backward:
                loss.backward()
            return loss, original_loss, mean_entropy
    ...
    def training_step(self, batch: TensorDict):
        ...
        for micro_batch in micro_batches:
            loss, original_loss, mean_entropy = self._compute_loss_and_backward(batch=micro_batch)
            ...
            # add
            step_entropy += mean_entropy.item() / n_micro_batches
        ...
        # add 
        step_entropy_tensor = torch.tensor(step_entropy).to(self.device_name)

        if is_cuda_available:
            torch.distributed.all_reduce(step_loss, op=torch.distributed.ReduceOp.AVG)
            # add
            torch.distributed.all_reduce(step_entropy_tensor, op=torch.distributed.ReduceOp.AVG)
        elif is_npu_available:
            torch.distributed.all_reduce(step_loss)
            step_loss /= self.device_mesh.size(0)
            # add
            torch.distributed.all_reduce(step_entropy_tensor)
            step_entropy_tensor /= self.device_mesh.size(0)
        return {"train/loss": step_loss.detach().item(), "train/entropy": step_entropy_tensor.detach().item(), "train/lr(1e-3)": lr * 1e3, "train/original_loss": step_original_loss}

    def validation_step(self, batch: TensorDict):
        self.fsdp_model.eval()
        with torch.no_grad():
            loss, original_loss, mean_entropy = self._compute_loss_and_backward(batch, do_backward=False)
            if is_cuda_available:
                torch.distributed.all_reduce(loss, op=torch.distributed.ReduceOp.AVG)
                torch.distributed.all_reduce(original_loss, op=torch.distributed.ReduceOp.AVG)
                torch.distributed.all_reduce(mean_entropy, op=torch.distributed.ReduceOp.AVG)
            elif is_npu_available:
                torch.distributed.all_reduce(loss)
                loss /= self.device_mesh.size(0)
                original_loss /= self.device_mesh.size(0)
                torch.distributed.all_reduce(mean_entropy)
                mean_entropy /= self.device_mesh.size(0)
        return loss, original_loss, mean_entropy
            ...
            for data in tqdm(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        val_loss, val_original_loss, val_entropy = self.validation_step(val_data)
                    ...
                    if rank == 0:
                        ...
                        val_entropy = torch.mean(torch.stack(val_entropies))
                        metric = {"val/loss": val_loss.detach().item(), "val/original_loss": val_original_loss.detach().item(), "val/entropy": val_entropy.detach().item()}
                        tracking.log(data=metric, step=global_step)
