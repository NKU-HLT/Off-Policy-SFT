...
class FSDPSFTTrainer:
    ...
    def training_step(self, batch: TensorDict):
        ...
        for micro_batch in micro_batches:
            ...
            # add
            loss, mean_entropy = self._compute_loss_and_backward(batch=micro_batch)
        ...
        return {"train/loss": step_loss.detach().item(), "train/entropy": step_entropy_tensor.detach().item(), "train/lr(1e-3)": lr * 1e3}

    def validation_step(self, batch: TensorDict):
        self.fsdp_model.eval()
        with torch.no_grad():
            loss, mean_entropy = self._compute_loss_and_backward(batch, do_backward=False)
            if is_cuda_available:
